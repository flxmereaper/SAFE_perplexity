...
class Model:
  ...
  def load(
    ...
    elif model_name.lower().startswith('perplexity:'):
      if not shared_config.perplexity_api_key:
        utils.maybe_print_error('No Perplexity API Key specified.')
        utils.stop_all_execution(True)

      if not _PERPLEXITY_MODELS.__contains__(model_name[11:]):
